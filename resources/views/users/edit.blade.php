    ...
    <form action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
                ...
                <h1 class="font-['Outfit'] font-black text-2xl md:text-4xl text-white tracking-tight">Editar Cadastro</h1>
                <p class="text-slate-400 text-xs md:text-sm mt-0.5">Atualize as informações pessoais, graduação e plano de <strong>{{ $user->name }}</strong>.</p>
            </div>
        ...
        <div class="bg-[#111726] border border-white/10 rounded-2xl p-6 shadow-xl mb-6">
            <div class="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div class="relative group cursor-pointer" onclick="document.getElementById('photo_input').click()" title="Clique para alterar a foto">
                ...
                <div class="flex-1 text-center sm:text-left">
                    <h3 class="font-['Outfit'] font-bold text-lg text-white">Foto de Perfil</h3>
                    ...
                    <button type="button" onclick="document.getElementById('photo_input').click()" class="inline-flex items-center gap-2 text-xs font-bold text-rose-400 hover:text-rose-300 transition-colors">
                        ...
                        {{ $user->photo ? 'Alterar foto atual' : 'Carregar nova imagem' }}
                    </button>
            ...
            <div class="lg:col-span-2 bg-[#111726] border border-white/10 rounded-2xl p-6 shadow-xl">
                <div class="flex items-center gap-3 pb-4 mb-6 border-b border-white/10">
                    ...
                    <div>
                        ...
                        <p class="text-slate-400 text-xs">Informações cadastrais básicas do atleta</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="data_nascimento" class="w-full bg-[#090d16] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-rose-500 transition-all" value="{{ old('data_nascimento', $user->data_nascimento ? $user->data_nascimento->format('Y-m-d') : '') }}">
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-300 mb-1.5">Status da Conta <span class="text-rose-500">*</span></label>
                        <select name="status" class="w-full bg-[#090d16] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-rose-500 transition-all" required>
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-300 mb-1.5">Alterar Senha <span class="text-slate-500 font-normal">(deixe em branco para manter a atual)</span></label>
                        <input type="password" name="password" class="w-full bg-[#090d16] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-rose-500 transition-all" placeholder="Preencha apenas se desejar redefinir">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-[#111726] border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col justify-between">
                <div>
                    <div class="flex items-center gap-3 pb-4 mb-6 border-b border-white/10">
                        ...
                        <div>
                            ...
                            <p class="text-slate-400 text-xs">Dados técnicos no tatame</p>
                        </div>
                    </div>
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-1.5">Faixa Atual <span class="text-rose-500">*</span></label>
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" class="w-full bg-[#090d16] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-rose-500 transition-all" value="{{ old('start_date', $user->start_date ? $user->start_date->format('Y-m-d') : '') }}" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 pt-4 border-t border-white/10">
                    <div class="flex items-center gap-3 text-slate-400 text-xs">
                        ...
                        <span>O grau é exibido na ficha do aluno e nas listas de chamada.</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-[#111726] border border-white/10 rounded-2xl p-6 shadow-xl mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-4 mb-6 border-b border-white/10">
                ...
                <div class="px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-semibold">
                    A alteração afeta as próximas faturas
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <label class="cursor-pointer group relative">
                    ...
                    <div class="p-5 rounded-2xl bg-[#182234] border border-white/10 peer-checked:border-cyan-500 peer-checked:bg-cyan-500/10 transition-all flex flex-col justify-between h-full">
                        <div>
                            <span class="text-xs font-bold uppercase tracking-wider text-slate-400 peer-checked:text-cyan-400 block mb-1">{{ $plan->name }}</span>
                            <div class="font-['Outfit'] font-black text-2xl text-white group-hover:text-cyan-300 transition-colors">
                                R$ {{ number_format($plan->price, 0, ',', '.') }}<span class="text-xs text-slate-500 font-medium">/mês</span>
                            </div>
                        </div>
                        <div class="mt-4 pt-3 border-t border-white/10 flex items-center gap-2 text-xs font-semibold text-slate-400 peer-checked:text-cyan-400">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-6 rounded-2xl bg-[#111726] border border-white/10 shadow-xl">
            <p class="text-slate-400 text-xs text-center sm:text-left">
                As alterações realizadas no cadastro de <strong>{{ $user->name }}</strong> entram em vigor imediatamente no sistema.
            </p>
